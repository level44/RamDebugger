...
proc RamDebugger::ExitGUI {} {
	...
	set options($i) ""
	if { [winfo exists $panedw] && [winfo width [$panedw getframe 0]] > 1 } {
	    set idx 0
...
proc RamDebugger::FillListBox {} {
	...
	if { [file isdir $fullpath] } {
	    ...
	    incr idxfolder
	} elseif { [regexp {(\.tcl|\.tml|\.h|\.c|\.cc|Makefil.*[^~])$} $i] } {
...
#         set indent 0
#     }
    if { $filetype == "c++" && $FirstChar == "\{" && $type == "c" } {
